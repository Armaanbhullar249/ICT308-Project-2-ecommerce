(function () {
  ...
  function recordSearch(query) {
    ...
    if (isLoggedIn() && window.WarnersBackend) {
      window.WarnersBackend.request("POST", "api/activity.php", { action: "search", query: q });
    }
  }
  ...
  function removeSearchHistoryItem(query) {
    ...
    if (isLoggedIn() && window.WarnersBackend) {
      window.WarnersBackend.request("POST", "api/activity.php", { action: "removeSearch", query });
    }
  }
  ...
  function clearSearchHistory() {
    ...
    if (isLoggedIn() && window.WarnersBackend) {
      window.WarnersBackend.request("POST", "api/activity.php", { action: "clearSearch" });
    }
  }
  ...
  function getRecommendations(seedProductId) {
    ...
    seeds.forEach((seed, index) => {
      ...
      getRules().forEach((rule) => {
        if (rule.whenId && seed.id === rule.whenId && rule.thenId) {
          bump(rule.thenId, 8 * recency);
          return;
        }
  ...
  function logout() {
    if (window.WarnersBackend) {
      window.WarnersBackend.request("POST", "api/auth.php", { action: "logout" });
    }
    clearSession();
  ...
  function renderHeader(active) {
    ...
    const dashOn =
      role === "admin"
        ? ["dashboard", "products", "categories", "rules", "reports", "settings"].includes(active)
